...
_INTENT_PATTERNS: list[tuple[str, re.Pattern]] = [
    ("emergency_stop", re.compile(
        r"\b(dừng khẩn cấp|dừng ngay lập tức|dừng ngay|khẩn cấp|"
        r"emergency stop|abort|kill motors|cut motors|kill throttle|mayday)\b",
        re.IGNORECASE
    )),
    ...
    ("hover",         re.compile(r"\b(đứng yên|giữ vị trí|giữ nguyên|hover|hold position|hold altitude|stay in place|maintain (position|altitude)|wait here)\b", re.IGNORECASE)),
    ...
    ("move_backward", re.compile(r"\b(lùi|bay lùi|lùi lại|backward|back up|move back)\b", re.IGNORECASE)),
    ("move_left",     re.compile(r"\b(sang trái|bay sang trái|trái|strafe left|slide left)\b", re.IGNORECASE)),
    ("move_right",    re.compile(r"\b(sang phải|bay sang phải|phải|strafe right|slide right)\b", re.IGNORECASE)),
    ...
    ("get_battery",   re.compile(r"\b(hỏi pin|kiểm tra pin|xem pin|pin còn|battery|how much battery|battery level)\b", re.IGNORECASE)),
    ("get_altitude",  re.compile(r"\b(hỏi độ cao|kiểm tra độ cao|độ cao hiện tại|altitude|how high|current altitude)\b", re.IGNORECASE)),
    ...
    ("stop",          re.compile(r"\b(dừng lại|ngừng lại|dừng|ngừng|stop|halt|pause|freeze|cut engines)\b", re.IGNORECASE)),
    ("move_forward",  re.compile(r"\b(tiến|tới|bay tới|tiến tới|đi thẳng|bay thẳng|forward|straight ahead|go straight)\b", re.IGNORECASE)),
]

_RE_DIST_M   = re.compile(r"(\d+(?:\.\d+)?)\s*(mét|met|meter|metre|m)s?\b", re.IGNORECASE)
_RE_DIST_CM  = re.compile(r"(\d+(?:\.\d+)?)\s*(centimet|phân|centimeter|centimetre|cm)s?\b", re.IGNORECASE)
...
def extract_entities(text: str, intent: Optional[str]) -> dict:
    ...
    if m := _RE_DIST_M.search(t):
        ...
        entities["distance_cm"] = int(float(m.group(1)))
