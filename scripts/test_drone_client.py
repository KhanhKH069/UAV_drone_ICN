...
def _print_result(msg: dict):
    ...
    if t == "partial":
        ...
        en = msg.get("en_text", "")
        req_conf = msg.get("require_confirmation", False)

        print(f"\n  ✅ COMMAND RECOGNIZED ({latency:.0f}ms):")
        if req_conf:
            print(
                "     ⚠️  CẢNH BÁO AN TOÀN: Lệnh nguy hiểm (LLM Fallback). Yêu cầu xác nhận từ phi công!"
            )
        ...
        print(
            f"  → MAVLink sẽ nhận: {json.dumps({'cmd': intent, **entities}, ensure_ascii=False)}"
        )
    elif t == "unknown":
        raw = msg.get("raw_text", "")
        latency = msg.get("latency_ms", 0)
...
async def test_with_text(text: str, lang: str):
    ...
    try:
        ...
        print(
            "  → Đảm bảo server đang chạy: docker-compose -f docker-compose.drone.yml up -d"
        )


async def test_with_wav(wav_path: str, lang: str, use_opus: bool = False):
        ...
        async with websockets.connect(uri) as ws:
            ...
            for i in range(0, len(audio_np), chunk_size):
                ...
                if use_opus:
                    ...
                    buf = io.BytesIO()
                    sf.write(buf, chunk_np, SAMPLE_RATE, format="OGG", subtype="OPUS")
                    chunk = buf.getvalue()
                else:
...
async def test_with_mic(lang: str, use_opus: bool = False):
                ...
                try:
                    while is_listening:
                        ...
                        if use_opus:
                            ...
                            buf = io.BytesIO()
                            sf.write(
                                buf, chunk_np, SAMPLE_RATE, format="OGG", subtype="OPUS"
                            )
                            chunk = buf.getvalue()
                        else:
                            chunk = chunk_bytes
...
def main():
    ...
    parser.add_argument(
        "--host", default="localhost", help="IP của Edge Server (mặc định: localhost)"
    )
    parser.add_argument(
        "--port", default=8765, type=int, help="WebSocket port (mặc định: 8765)"
    )
    parser.add_argument(
        "--lang", default="en", choices=["en", "vi"], help="Ngôn ngữ lệnh"
    )
    ...
    parser.add_argument(
        "--opus", action="store_true", help="Nén audio bằng chuẩn OPUS trước khi gửi"
    )
    ...
    if args.text:
        ...
        asyncio.run(test_with_wav(args.audio, args.lang, args.opus))
    elif args.mic:
        asyncio.run(test_with_mic(args.lang, args.opus))
    else:
        print("\n📖 Ví dụ sử dụng:")
